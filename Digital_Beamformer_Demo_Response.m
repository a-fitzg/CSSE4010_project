...
% Design and Implementation of a wideband digital beaformer

% Demonstration of a wideband digital beamformer with audio signals
% received by a uniform linear array of microphones

% This script simulates the frequency response of the 2D FIR filter

clear all; close all; clc;

Na=17; % Number of antennas/sensors - has to be odd
NN=2048; % number of time samples taken from the audio signals
...
SIG=fftshift(fft2(sig));
SIG=SIG/max(max(abs(SIG)));

figure
mesh(20*log10(abs(SIG))); title('2D Magnitude Spectrum of the Input Signal');
xlabel('\omega_x'); ylabel('\omega_t'); view(2);
...
Mt=17; % Temporal order of the filter
Mx=Na; % Spatial order of the filter
h=zeros(size(sig));
h(1:Mx,1:Mt)=fir2dpln(Mx,Mt,sin(DOA1),0.01); % Generating the 2D FIR filter coeff
H=fftshift(fft2(h));
figure
mesh(abs(H)'); title('2D Frequency Response of the Filter'); xlabel('\omega_x'); ylabel('\omega_t'); view(2);
...
function [w] = ULA_planewave(x,DOA,Na)
    ...
    % signal at each sensor.
        ...
        % tau=(dx/c)*sin(DOA); see class notes for details.
    for p = 2:Na
